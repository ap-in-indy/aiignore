...
export function evaluateCodexPreToolUse(
  policy: LoadedPolicy,
  ...
  const networkCandidates = collectNamedStrings(input.tool_input, /(^|_)(urls?|uris?)$/iu);
  ...
  if (!blocking && !failure) return { denied: false, decisions, errors, response: null };
...
function toolOperation(toolName: string | undefined): FileOperation {
  if (toolName && /apply_patch|copy|create|delete|edit|mkdir|move|remove|rename|replace|touch|write/iu.test(toolName)) return 'write';
  if (toolName && /bash|shell|exec|run_/iu.test(toolName)) return 'execute';
  if (toolName && /find|glob|grep|list|search/iu.test(toolName)) return 'discover';
  return 'read';
}
