...
describe('release SBOM normalization', () => {
  it('removes npm run-specific identity and time while preserving deterministic content', () => {
    ...
    const identity = { name: 'aiignore', version: '0.1.0-alpha.1' };
    ...
    expect(normalized).toContain('"name": "aiignore"');
  ...
  it('rejects documents that are not npm CycloneDX SBOMs', () => {
    ...
    expect(() =>
      normalizeSbom(inconsistent, { name: 'aiignore', version: '0.1.0-alpha.1' })
...
function fixture(serialNumber: string, timestamp: string) {
  return {
    ...
    metadata: {
      timestamp,
      component: {
        'bom-ref': 'aiignore@0.1.0-alpha.1',
        purl: 'pkg:npm/aiignore@0.1.0-alpha.1',
    ...
    dependencies: [{ ref: 'aiignore@0.1.0-alpha.1', dependsOn: ['fixture@1.0.0'] }]
  };
}
